"""
Airflow DAG to run the job crawler (TopCV spider).
...
with DAG(
    dag_id="job_crawler_dag",
    description="Run TopCV crawler via Scrapy",
    ...
    params={
        "max_pages": Param(
            default=5,  # Default from rate_limiting_config
            type="integer",
            minimum=1,
            maximum=200,  # Increased from 50 to 200
            description="Số trang tối đa để crawl từ TopCV (giảm để tránh rate limiting)"
        ),
    },
) as dag:
    ...
    try:
        ...
        # Get rate limiting settings for TopCV
        rate_limiting_settings = get_scrapy_settings('topcv.vn')
        ...
        default_max_pages = get_max_pages_for_domain('topcv.vn')
    ...
    crawl_topcv = BashOperator(
        ...
        bash_command=(
            ...
            "/home/airflow/.local/bin/scrapy crawl topcv_spider "
            "-a max_pages={{ params.max_pages | default('" + str(default_max_pages) + "') }} "
            f"{scrapy_settings_str} "
        ),
        execution_timeout=timedelta(hours=12),  # Increased timeout to allow longer crawls
    )
    
    # Skill post-processing disabled; only run crawl_topcv
